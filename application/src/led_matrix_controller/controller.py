...
class LEDMatrixController:
    ...
    def camera_input(self, camera_id: int = 0):
        """Live camera input."""
        if not HAS_CV2:
            raise ImportError("OpenCV required: pip install opencv-python")
        ...
        print(f"Camera {camera_id} active")
        print("Press Ctrl+C to stop")
    ...
    def scroll_text(
        ...
        try:
            while True:
                image = np.zeros((self.height, self.width, 3), dtype=np.uint8)
                cv2.putText(image, text, (int(x_pos), y_pos), font, font_scale, color, thickness)
