...
""",
    ...
    device_group.add_argument(
        "--port", "-p", default=None, help="Serial port (auto-detect if not specified)"
    )
    device_group.add_argument(
    ...
    input_mutex.add_argument("--image", "-i", metavar="FILE", help="Display image file")
    input_mutex.add_argument("--video", "-v", metavar="FILE", help="Play video file")
    ...
    input_mutex.add_argument(
        ...
        dest="dem_taichi",
        action="store_true",
        help="Launch high-speed 2D-DEM rotating drum simulation (Taichi GPU, 60-120+ FPS)",
    )
    input_mutex.add_argument(
        "--dem-matrix",
        dest="dem_matrix",
        action="store_true",
        help="Launch 2D-DEM simulation in authentic 64x64 LED matrix pixel/voxel display mode",
    )
    input_mutex.add_argument(
        "--fill", "-f", metavar="R,G,B", help="Fill with solid color (e.g., 255,0,0 for red)"
    )

    # Display options
    display_group = parser.add_argument_group("Display options")
    display_group.add_argument("--loop", action="store_true", help="Loop video playback")
    display_group.add_argument(
        "--fps", type=float, default=30.0, help="Target FPS for demos (default: 30)"
    )
    display_group.add_argument(
        "--brightness", type=float, default=1.0, help="Brightness multiplier 0.0-1.0 (default: 1.0)"
    )
    ...
    if not any(
        ...
        parser.print_help()
        print("\nError: Please specify an input source:")
        print("  (--image, --video, --demo, --dem, --dem-matrix, or --fill)")
        sys.exit(1)

    if args.dem_taichi or args.dem_matrix:
        from .demo_taichi_dem import run_taichi_gui

        run_taichi_gui(matrix_mode=args.dem_matrix)
        return
    ...
    try:
        ...
        controller = LEDMatrixController(device=device, brightness=args.brightness)
