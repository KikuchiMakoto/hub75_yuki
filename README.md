...
- **高速・安定表示**: PIO (7.14 MHz 最適化) + DMA による高速シフト出力（内部リフレッシュレート約 350 Hz、フリッカーフリー）
- **デュアルコア設計**: Core0 で USB 受信／DEM 物理演算＋BCM 変換、Core1 で 100% パネル駆動専任（完全チラつき防止）
- **ハードウェア除算アクセラレーション**: RP2040 SIO 32-bit 除算器による DEM 物理演算の高速化（約 250 FPS）
- **過大評価描画**: 2x2 box pixel の隙間・角抜けを排除した高密度・高輝度砂粒表示
- **即時起動**: 余計な起動スプラッシュやセルフテスト待ちを全廃し、電源投入直後から即時動作
- **RGB565フォーマット**: 16ビットカラー（5-6-5ビット）による効率的な転送
- **COBSエンコーディング**: 0x00区切りのバイナリフレーム転送
- **高画質**: 6ビットBCM（64階調）+ Gamma補正（2.2）
- **最新コードベース**: 不要な低速 CPU GPIO 駆動コードを全廃し、PIO + DMA 専用に一本化
